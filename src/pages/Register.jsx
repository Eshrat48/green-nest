...
export default function Signup() {
  ...
  function handleSignup(e) {
    ...
    if (!validatePassword(password)) {
      setError("Password must have at least one uppercase, one lowercase letter, and be at least 6 characters.");
      return;
    }
    if (!name || !email || !photo) {
    ...
    <div className="min-h-screen bg-[#ddf0e4ff] flex items-center justify-center">
      ...
      >
        ...
        <div className="mb-5 h-22 w-22">
          <img src={leafLogo} alt="GreenNest Logo" className="h-10 w-10 mx-auto" />
        </div>
        <h2 className="text-center font-semibold text-green-800 text-[25px]">Create Account</h2>
        <p className="text-[16px] text-gray-500 text-center">
          Join GreenNest and start your plant journey
        </p>
        {/* Name */}
        <div className="w-full mb-4">
          <label className="text-sm block text-left mb-1">Name</label>
        ...
        {/* Photo URL */}
        <div className="w-full mb-4">
          <label className="text-sm block text-left mb-1">Photo URL</label>
          <div className="relative">
            <span className="absolute left-3 top-3.5 text-gray-400"><FiImage /></span>
            <input
              className="pl-10 pr-3 py-2 border border-gray-200 rounded-md outline-none w-full focus:border-green-400 transition"
              type="url"
              required
